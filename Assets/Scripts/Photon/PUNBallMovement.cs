...
public class PUNBallMovement : MonoBehaviour
{
    ...
    public float baseLaunchPower = 2.0f;
    public float baseImpactPower = 1.5f;
    ...
    {
        //Debug.Log(direction);
        direction = new Vector3(direction.x * Mathf.Cos(Mathf.Deg2Rad * angle), ((45 - angle) / 45.0f + MovementSpeed* baseLaunchPower) * Mathf.Sin(Mathf.Deg2Rad * angle) / (MovementSpeed * baseLaunchPower), direction.z * Mathf.Cos(Mathf.Deg2Rad * angle));
        //Debug.Log(direction);
        Vector3 _impulse = direction * (dragForce * rigidbody.mass * MovementSpeed * baseLaunchPower);
        ...
        if (collision.gameObject.GetComponent<PUNBallMovement>() != null && photonView.IsMine)
        {
            ...
            if (collision.gameObject.GetComponent<Rigidbody>().velocity.sqrMagnitude > MinimalImpactForce * MinimalImpactForce && rigidbody.velocity.sqrMagnitude > MinimalImpactForce * MinimalImpactForce)
            {
                ...
                if (amplify == CollideStates.Giver)
                {
                    ...
                    float sqrSpeed = Mathf.Clamp((rigidbody.velocity * ImpactGivingCoef * impactPower * baseImpactPower - Vector3.up * rigidbody.velocity.y * (ImpactGivingCoef - 1)).sqrMagnitude, 0, MaxSpeed * MaxSpeed);
                    rigidbody.velocity = (rigidbody.velocity * ImpactGivingCoef * impactPower * baseImpactPower - Vector3.up * rigidbody.velocity.y * (ImpactGivingCoef - 1)).normalized * Mathf.Sqrt(sqrSpeed); ;
                }
                else
                {
                    //Debug.Log("Recieving Collider " + gameObject.name);
                    float sqrSpeed = Mathf.Clamp((rigidbody.velocity * ImpactRecievingCoef * impactPower * baseImpactPower - Vector3.up * rigidbody.velocity.y * (ImpactRecievingCoef - 1)).sqrMagnitude, 0, MaxSpeed * MaxSpeed);

                    rigidbody.velocity = (rigidbody.velocity * ImpactRecievingCoef * impactPower * baseImpactPower - Vector3.up * rigidbody.velocity.y * (ImpactRecievingCoef - 1)).normalized * Mathf.Sqrt(sqrSpeed);
                }
            }
        }
    }
